...
router.post('/crud', (req,res,next) =>{
    ...
    switch(choice){
        ...
        case 1:
            //Agregar
            crud.simple(crud.add_product(data.quant, data.name, data.prod, data.product_precio, data.prod))
            .then((r)=>{
                res.send({msg: 'Exito eliminando!', status: 200});
            })
            .catch((err)=>{
        ...
        case 3:
            //Actualizar
            crud.simple(crud.upd_product(data.id, data.name, data.quant, data.prod, data.product_precio))
            .then((r)=>{
